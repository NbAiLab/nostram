...
for model_path in "${model_paths[@]}"; do
    # Loop over each split
        echo "Running model: $model_path on NbAiLab/ncc_speech_v7  and split: test_nst"
        python test_dataset.py --device 1 --num_beams 5 --from_flax --dataset_path NbAiLab/ncc_speech_v7 --split "test_nst" --model_path "$model_path" --calculate_wer --save_file /home/perk/results.jsonl
done
...
for model_path in "${model_paths[@]}"; do
    # Loop over each split
        ...
        python test_dataset.py --device 1 --num_beams 5 --from_flax --dataset_path NbAiLab/ncc_speech_v7 --split "test_norwegian_fleurs" --model_path "$model_path" --calculate_wer --save_file /home/perk/results.jsonl
done
