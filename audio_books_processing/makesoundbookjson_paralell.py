...
outputdir=""
masteraudioinputdir=""
masteraudiooutputdir=""
masteraudioextractdir=""
...
def extractfromsoundfile_ffmpeg_simple(inputmp3,outputdir,e):
...
def processbook(extractfile):
            ...
            if duration > 30 and len(splitcodes) > 3:
                ...
                for cnt, part in enumerate(stringparts):
                    if (cnt == 0):
                        ...
                        print(splitcodes)
                        print(splitcodes[cnt].split(",")[2].split("/")[0] + ":" + str(
                            splitcodes[cnt + 1].split(",")[1]) + ":" + part)
                        print(splitcodes)
                        currentstartsecond = float(splitcodes[cnt].split(",")[2].split("/")[0])
    ...
    print(size)
    # 4294967296
    mkdirifnotexists(outputdir + "/mp3")
    Parallel(n_jobs=15)(delayed(extractfromsoundfile_ffmpeg_simple)(masteraudioinputdir + "/" + soundfile, outputdir + "/mp3", i) for i in splitlist)
...
if __name__ == "__main__":
    ...
    for extractfile in glob.glob(masterextractinputdir + "/*.txt"):
        processbook(extractfile)

    #Parallel(n_jobs=10)(delayed(processbook)(i) for i in infiles)
