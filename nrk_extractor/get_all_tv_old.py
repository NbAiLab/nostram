...
def main(args):
    ...
    with jsonlines.open(tv_file, mode='w') as writer:
            ...
            for category in categories_json[pageListItems]:
                ...
                murl = "https://psapi.nrk.no/"+category["_links"]["self"]["href"]
                
                r = requests.get(murl)
                if r.status_code != 200:
                    raise Exception("Failed to load metadata from '%s'" % murl)
                serie_json = json.loads(r.text)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', help="Complete path to csv output file. Note that it will split this in two names with _singles and _series by itself", required=True)
