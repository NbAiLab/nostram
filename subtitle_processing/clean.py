...
def main(args):
    ...
    print(f'*** Data loaded with {len(data)} subtitles. Log written to {os.path.join(args.output_folder, "log/", log_name)}. ({exec_time()})')
    ...
    logger.info(f'*** Finished processing file. Result has {len(data)} subtitles. Result is written to {os.path.join(args.output_folder, os.path.basename(args.input_file))}. ({exec_time()})')
